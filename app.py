...
# Simulation function
def simulate_roulette_number(n_rounds, bet_amount, probabilities, player_choices, starting_balance, tweaked=False):
...
player_choices = st.multiselect(
    "Choose number(s) to bet on (0-36):",
    options=list(range(37)),
    default=[17]
)
...
if run_fair:
    if len(player_choices) == 0:
        st.warning("Please select at least one number to bet on.")
    else:
        st.subheader("FAIR Game Results")
        fair_df = simulate_roulette_number(n_rounds, bet_amount, fair_prob, player_choices, starting_balance)
...
if run_tweaked:
    if len(player_choices) == 0:
        ...
        tweaked_df = simulate_roulette_number(n_rounds, bet_amount, tweaked_prob, player_choices, starting_balance, tweaked=True)
